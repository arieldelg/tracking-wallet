...
const WarningView = () => {
  const { setCloseModalDelete, activeAccount } = useWalletStore();
  ...
  return (
    <Form
      onSubmit={(e) => {
        e.preventDefault();
        submit(
          { delete: activeAccount?._id as string },
          {
            method: "post",
            action: "/accounts",
          }
      ...
      className="bg-customBGDark1 rounded-2xl ring-2 px-4 md:px-8 py-5 ultraWide:p-8 text-lg ultraWide:text-xl animate-fadeInBillModal ring-white min-w-[400px] xl:min-w-[600px] ultraWide:min-w-[730px] max-h-[600px] ultraWide:max-h-[750px] h-5/6 flex flex-col ultraWide:justify-evenly justify-evenly w-[400px] space-y-10"
    >
      ...
      <div className="text-center text-2xl tracking-widest">
        <p>
          ...
          <span className="first-letter:capitalize text-3xl text-red-500">
            {activeAccount?.title}
          </span>
          ?
        </p>
